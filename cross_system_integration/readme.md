...
We recommend using as _system_ the covariate corresponding to the substantial batch effects (e.g., species, organoid-tissue, etc.; currently implemented only for exactly two systems - a preliminary version of an extension that works with any number of systems is on branch multiple_sys_model, but we are still testing it and can not guarantee for optimal integration results) and as _covariate keys_ any other covariates to be corrected for, such as batches within systems (samples or datasets).
