...
N_MAX_PRIOR_COMPONENTS = 100

# %%
balanced_ct_samples = (
    adata.obs
    .groupby([SYSTEM_KEY, CT_KEY])
    .apply(lambda x: x.sample(N_MAX_PRIOR_COMPONENTS)
                     .reset_index()
                     .rename(columns={'index': 'cell_id'}))
    .reset_index(drop=True)
    .set_index('cell_id')
)
balanced_ct_samples.head()
...
pseudoinputs_data_indices = (
    balanced_ct_samples[balanced_ct_samples[SYSTEM_KEY] == 0]
...
# %%
pseudoinputs_data_indices = (
    balanced_ct_samples[balanced_ct_samples[SYSTEM_KEY] == 1]
    ['uid'].sample(N_PRIOR_COMPONENTS)
)
pseudoinputs_data_indices
