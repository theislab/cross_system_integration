...
# ### Seurat v4 API ###
# sdata <- FindVariableFeatures(sdata, nfeatures = n_hvg)

# # Splitting based on batch (batch covers system)
# seurat_list <- SplitObject(sdata, split.by = args$batch_key)
# seurat_list <- lapply(X = seurat_list, FUN = function(x) {
#   x  <- ScaleData(x)
#   x <- RunPCA(x, npcs = n_latent)
#   return(x)
# })
...
sdata <- FindVariableFeatures(sdata, nfeatures = n_hvg)  # Make all variables as hvg since we already did hvg selection
